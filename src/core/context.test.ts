...
Deno.test("a type variable and a term variable may share a name", () => {
  const context = new Context();
  const type = context.pushUniversal("x", TUnknown);
...
Deno.test("push hands back the level it allocated", () => {
  const context = new Context();
  expect(context.pushUniversal("X", TUnknown)).toBe(mkLevel(0));
...
Deno.test("solve records a solution in place", () => {
  ...
  expect(context.solve(a, TData(Bool))).toBeUndefined();
  expect(alphaEq(context.solutionOf(a) ?? TUnknown, TData(Bool))).toBe(true);
});

Deno.test("solve rejects a solution mentioning the variable itself", () => {
  const { context, a } = withEVar();

  const failure = context.solve(a, TFun([], [EVar(a, "a")], TUnknown));
  expect(failure?.kind).toBe("occurs");
  expect(context.solutionOf(a)).toBeUndefined();
});
...
Deno.test("solve rejects a solution that escapes its scope", () => {
  ...
  expect(context.solve(a, FVar(X, "X"))?.kind).toBe("escapes");
});
...
Deno.test("solve accepts a solution mentioning something to its left", () => {
  ...
  expect(context.solve(a, FVar(X, "X"))).toBeUndefined();
  expect(alphaEq(context.solutionOf(a) ?? TUnknown, FVar(X, "X"))).toBe(true);
});

Deno.test("solve refuses to overwrite an existing solution", () => {
  const { context, a } = withEVar();
  context.solve(a, TData(Bool));

  expect(context.solve(a, TUnknown)?.kind).toBe("alreadySolved");
  expect(alphaEq(context.solutionOf(a) ?? TUnknown, TData(Bool))).toBe(true);
});
...
Deno.test("solve reports a level that is not an EVar", () => {
  const context = new Context();
  const X = context.pushUniversal("X", TUnknown);
  expect(context.solve(X, TUnknown)?.kind).toBe("unbound");
  expect(context.solve(mkLevel(9), TUnknown)?.kind).toBe("unbound");
});
...
Deno.test("apply follows a chain of solutions", () => {
  ...
  const a = context.pushEVar("a");
  context.solve(b, TData(Bool));
  context.solve(a, EVar(b, "b"));
...
Deno.test("truncate ends a scope, keeping what came before it", () => {
  ...
  expect(context.since(mark).length).toBe(2);
...
Deno.test("truncate reuses the levels it dropped", () => {
  ...
  const X = context.pushUniversal("X", TUnknown);
  context.truncate(mark);
  expect(context.pushUniversal("Y", TUnknown)).toBe(X);
});
...
Deno.test("assertClosed throws on a type that outlives its scope", () => {
  ...
  const X = context.pushUniversal("X", TUnknown);
  context.truncate(mark);
  expect(() => context.assertClosed("test", [FVar(X, "X")], mark)).toThrow(
    "escaped a scope",
  );
  expect(() => context.assertClosed("test", [TData(Bool)], mark)).not.toThrow();
});
...
Deno.test("assertClosed allows the binders a stored type was closed into", () => {
  ...
  expect(() => context.assertClosed("field", [field], 0, 2)).not.toThrow();
  expect(() => context.assertClosed("field", [field], 0)).toThrow();
});
