...
Deno.test("stdlib: one parameter list stages itself where it has to", () => {
  ...
  // parameter's *scope* is still the list that binds it, so writing `B` on the
  // first list is still wrong; where it is bound alongside the arguments that
  ...
  // Both in one list: `op` waits on `B`, nothing else waiting supplies it, so
  // `z` goes first and `op` is checked against its answer. The cut falls where
  // the second list used to be.
  expect(
    restagedFold(
  ...
  // avoid: `xs` and `z` are checked first, `op` after both.
  expect(
    restagedFold(
