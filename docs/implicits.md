...
to be undone. Speculation was already ruled out once, in the recursion note
above; this is the same absence, and it is deliberate both times.
...
one offered to the TS rounds idea above, and it keeps the invariant in both
cases by being strictly weaker: there, a lambda that needs its parameter's
structure; here, `create`. Scala can decline it and we cannot, which is the
