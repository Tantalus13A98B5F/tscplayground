...
**Refuted. Implemented, and not merged.** Stages 1-3 below all landed, green, on
a branch; the design then lost to the thing it was a proxy for, and the branch
is closed. This file stays because the implementation answered several questions
that outlive it -- how a super type enters the variance fixed point, why an
eager parent image is right here and wrong in an OO language, why a
`match ... as` is forced once names are qualified -- and because the reason it
lost is the useful part.

_Why it lost._ Three arguments, in the order they arrived.

**Nobody else does it.** TypeScript and F# both have subtyping and discriminated
unions, and in both the unions stay out of the subtype relation; TS's
case-filtering comes from general unions rather than from a declared hierarchy.
Scala has it, but restricted to depth one, and pays for it by weakening match
exhaustiveness. The precedent that most resembles this design is OCaml's
polymorphic variants, which is a cautionary one. A feature absent from every
neighbour with the same ingredients is not obviously a gap.

**It was a proxy for a smaller feature.** The motivating example was `Show[Dog]`
-- wanting `Dog <: Animal` so that implicit selection had something to do.
`docs/implicits.md` works that through and reaches the opposite conclusion: a
declared bound does the same expressive job for free, and this hierarchy would
_undo_ the invariant that file spends. `#relateData` demanding
`s.name === t.name` is exactly what makes matching and subtype-search coincide
on a ground goal, so adding a hierarchy makes implicits more expensive, not
less.

**The refinement case has a cheaper answer.** `NonEmpty[A] <: List[A]` and
`Type <: TypePattern` are both really asking that a _constructor_ be a type of
its own -- `Cons[A] <: List[A]`, derived rather than declared, depth one, the
coercion the identity, and one new leaf in `#relateData`. That delivers the
refinement without super-constructor bodies, without an image in the value,
without qualified names, and without a `match ... as`. It is the successor, and
it is what is on the roadmap.
...
point that already handles two datatypes naming each other -- which is also why
this could not be a per-declaration pass ordered by the super-type relation.
`docs/variance.md` would gain one occurrence source and no new rule; since it
never did, the worked cases live here.

Against `datatype Box[A] where | MkBox(A)`, which is `+A`:
...
it is walked by §2's rules like anything else.
...
no `Both[A']` above `Both[A]` for the obligation to be about.
...
**4. What is left.** Never started; see the epitaph. Nothing named. The claims
that nothing else changed: `#eqtype`, `#checkMatch`, `#remaining` and avoidance
are listed above as untouched, and stayed so -- a scrutinee still has exactly
one datatype because there is no downcast, which is what the evaluator tests
lean on when they annotate.
